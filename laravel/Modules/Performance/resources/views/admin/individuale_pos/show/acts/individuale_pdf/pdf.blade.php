@include('ptv::pdf.css')
<page>
    <h4 style="text-align:center;">SISTEMA DI MISURAZIONE E VALUTAZIONE DEL RISULTATO - Anno
        {{ $row->anno }}
        <br />
    ...
    @php
        /*
        $criteri = $row->options->where('name', 'criterio')->where('parent_id', 0)->sortBy('pos');

        $criteri1 = $row->criteriValutazione->sortBy('posizione');
