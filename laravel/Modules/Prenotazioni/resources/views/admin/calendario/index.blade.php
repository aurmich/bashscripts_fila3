...
@foreach($rows as $row)
	...
	<thead>
		<tr>
			<td>Orario</td>
			<td>Posti Liberi</td>
			<td><a class="btn btn-small btn-info"  data-toggle="tooltip" title="prenota la data" href="{{
					route('prenotazioni.calendario.xlsDay',['giorno'=>$curr,'id_tipo'=>$row->id_tipo] ) }}">
				<i class="fa fa-file-excel-o fa-fw" aria-hidden="true"></i>&nbsp;</a>
			</td>
		</tr>
	</thead>
	@while($curr<$end)
		@php
			...
			$posti_occupati=$row->posti_occupati($curr);
		...
		<tr>
			...
			@if($posti_occupati>0)
				...
				<a class="btn btn-small btn-info"  data-toggle="tooltip" title="prenota la data" href="{{
					route('prenotazioni.calendario.xls',['giorno'=>$curr,'id_tipo'=>$row->id_tipo] ) }}">
				<i class="fa fa-file-excel-o fa-fw" aria-hidden="true"></i>&nbsp;</a>
				--}}
			@endif
			</td>
