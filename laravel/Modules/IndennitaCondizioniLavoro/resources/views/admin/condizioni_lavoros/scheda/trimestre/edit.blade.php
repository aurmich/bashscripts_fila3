...
@section('section')
    ...
    <a href="{{ route('indennita.condizionilavoro.scheda.trimestre.index', $params) }}"
        class="btn btn-default">&laquo;&nbsp;Back</a>
    <hr />
    ...
    <b>giorni presenza periodo:</b>{{ $row->gg_presenza_periodo }} <br />
    {{-- <br>[{{ $row->gg_presenza_periodo }}] --}}
    {{ Form::bsText('tot_presenza_periodo_plus_no_timbr') }}
    <br />
    <b>Trimestre N:</b> {{ $id_trimestre }} <b>Dal</b> {{ $row->dal->format('d/m/Y') }} - <b>Al</b>
    {{ $row->al->format('d/m/Y') }} <br />
